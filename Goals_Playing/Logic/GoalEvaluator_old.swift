//
//  GoalEvaluator_old.swift
//  Goals_Playing
//
//  Created by Adolfo Gerard Montilla Gonzalez on 20-05-26.
//
...
/*
 ...
 reachedGoalsByDay took: 2.027357166 seconds
 
 ✔ Test "Measure reachedGoalsByDay execution time" passed after 2.037 seconds.
 ✔ Suite GoalEvaluatorTests passed after 2.038 seconds.
 ✔ Test run with 1 test in 1 suite passed after 2.038 seconds.
 */
struct GoalEvaluatorOld
{
    ...
    private func sessionsCompleted(on day: Date, sessions: [StudySession]) -> TimeInterval
    {
        ...
        return sessions.reduce(0) { total, session in
            ...
            let overlapEnd = min(session.endDate ?? Date.now, dayInterval.end)
    ...
    private func daysCoveredBySessions(_ sessions: [StudySession]) -> [Date]
    {
        ...
        for session in sessions where session.startDate < session.endDate ?? Date.now {
            var currentDay = calendar.startOfDay(for: session.startDate)

            while currentDay < session.endDate ?? Date.now {
